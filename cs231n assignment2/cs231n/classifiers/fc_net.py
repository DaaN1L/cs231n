...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        # Сохраним размерности всех слоев в один список
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1):
            ...
            if self.normalization:
                ...
                if self.normalization == 'batchnorm':
                    scores, cache[i+1] = affine_bn_relu_forward(scores, W_i, b_i, gamma_i, beta_i, bn_param_i)
                elif self.normalization == 'layernorm':
                    scores, cache[i+1] = affine_ln_relu_forward(scores, W_i, b_i, gamma_i, beta_i, bn_param_i)

            elif self.normalization == None:
                scores, cache[i+1] = affine_relu_forward(scores, W_i, b_i)

            if self.use_dropout:
                scores, drop_cache[i+1] = dropout_forward(scores, self.dropout_param)

        W_last = self.params['W' + str(self.num_layers)]
        b_last = self.params['b' + str(self.num_layers)]
        
        scores, cache[self.num_layers] = affine_forward(scores, W_last, b_last)
        ...
        dout, grads[W_last], grads[b_last] = affine_backward(dx, cache[self.num_layers])
        grads[W_last] += self.reg * self.params[W_last]
        loss += np.sum(self.params[W_last] ** 2) * self.reg * 0.5
        ...
        for i in range(self.num_layers-1, 0, -1):
            ...
            if self.normalization:
                ...
                if self.normalization == "batchnorm":
                    ...
                    dout, grads[W_i], grads[b_i], grads[gamma_i], grads[beta_i] = affine_ln_relu_backward(dout, cache[i])

            elif self.normalization == None:
                dout, grads[W_i], grads[b_i] = affine_relu_backward(dout, cache[i])
            
            loss += np.sum(self.params[W_i] ** 2) * self.reg * 0.5
